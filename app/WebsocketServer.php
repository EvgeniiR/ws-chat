...
class WebsocketServer
{
    ...
    function addMessage(int $id, $data)
    {
        ...
        $response = (new MessagesResponse())->addMessage($username, $data->message)->getJson();
        foreach ($Ids as $id) {
     ...
     */
    function login(int $id, string $username)
    {
        if (empty($username)) {
        ...
        foreach ($this->users_table as $user) {
            if ($user['username'] == $username) {
                $userWithSomeNickId = $user['id'];
                $id = $this->global_channel->peek();
                if (($key = array_search($userWithSomeNickId, $id)) !== FALSE) {
                    if($id[$key] !== $id) {
